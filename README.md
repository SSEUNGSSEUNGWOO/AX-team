...
**코드 잘림 근본 수정**
- `plan_code_structure` 토큰 500 → 1500: 파일 구조 JSON이 파싱 실패하며 무조건 `main.py` 단일 파일로 폴백되던 버그 수정
- `write_project_docs` 문서 생성 토큰 1500 → 8192
- `doc_call` / `tool_agent_call` 에서 토론용 글자수 제한 프롬프트 제거 — 문서·코드 생성 시 발언 제한이 그대로 적용돼 4줄짜리 문서가 생성되던 버그 수정

**파일 분담 토론 도입**
- 파일 구조 확정 후 에이전트들이 직접 담당 파일을 선언하는 라운드 추가 (`[CLAIM: ...]` 파싱)
- 코드 생성 병렬 → **순차 실행**으로 변경: 병렬 생성 시 에이전트간 맥락 충돌 방지

**토론 발언 품질 개선**
- 모든 에이전트 프롬프트에 "토론 중 코드 작성 금지" 추가
- 토론 발언 max_tokens 상향 (문장 중간 절단 방지)

**API 키 로드 순서 수정**
- `utils.py`에서 `client = Anthropic()` 가 `load_dotenv()` 이전에 초기화되어 API 키를 못 읽던 버그 수정

**UI 개선**
- 에이전트별 개인 책상 추가 (색상 강조선으로 담당자 구분)
- 말풍선 마크다운(`**bold**` 등) 자동 제거
- 말풍선 화면 경계 넘침 방지 (좌우 clamp 로직)
- `DOMContentLoaded`에서 초기 위치 JS 주입 — HTML 인라인 스타일 브라우저 캐시 문제 해결
...
| 항목 | 내용 |
|------|------|
| 코드 일관성 | 파일 분담 토론 후 순차 생성하지만 에이전트간 맥락 공유가 완벽하지 않아 import 불일치 가능 |
| Rate Limit | Claude API 분당 출력 토큰 한도로 토론·생성이 많을수록 간헐적 실패 가능 |
